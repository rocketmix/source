...
public class SwaggerController {
	...
	private static final String LOCAL_SERVER = "ROUTINGSERVER";
	...
	public @ResponseBody byte[] css() throws IOException {
		...
		content = content.replaceFirst("\\.swagger-ui \\.topbar[ ]*\\{[^\\}]*\\}", ".swagger-ui .topbar{padding:8px 0;background-color: #000000;height: 52px;white-space: nowrap;}");
		content = content.replaceFirst("\\.swagger-ui \\.info[ ]*\\{[^\\}]*\\}", ".swagger-ui .info {margin: 0px 0px 0px 0px;}");
			...
			for (Application application : applications) {
				try {
					...
					for (InstanceInfo applicationsInstance : applicationsInstances) {
						...
						String jsonAPIDefinitionUrl = applicationsInstance.getHomePageUrl() + DOC_PATH;
	...
	public @ResponseBody byte[] proxy(@RequestParam String url, @RequestParam String vipaddress) throws IOException {
		...
		routingServer.addProperty("url", currentServerURL + "/" + vipaddress + "/services/");
